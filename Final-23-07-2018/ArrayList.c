...
int al_push(ArrayList* this, int index, void* pElement)
{
    ...
    if(this && pElement){
        int tam=this->len(this);
        if(((index>=0) && (index<tam)) || tam==index){
           expand(this,index);
...
ArrayList *al_localidad(ArrayList* this, int (*pFunc)(void*,char *localidad), char* localidad){
    ArrayList *salida=al_newArrayList();

    if(this && pFunc && salida){
        int len=this->len(this);
        for(int i=0;i<len;i++){
            void *aux=this->get(this,i);
            if(pFunc(aux,localidad)){
                salida->add(salida,aux);
            }
        }
    }
